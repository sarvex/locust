    ...
    def _start(self, user_count: int, spawn_rate: float, wait: bool = False, user_classes: list = None) -> None:
        ...
        try:
            for dispatched_users in self._users_dispatcher:
                ...
                logger.debug(
                    f"Ramping to {_format_user_classes_count_for_log(user_classes_count)}"
                )
...
class MasterRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_bind_host, master_bind_port):
        ...
        try:
            ...
            if e.args[0] == "Socket bind failure: Address already in use":
                port_string = (
                    f"{master_bind_host}:{str(master_bind_port)}"
                    if master_bind_host != "*"
    ...
    def start(
        ...
        worker_spawn_rate = spawn_rate / (num_workers or 1)
    ...
    def heartbeat_worker(self) -> NoReturn:
        while True:
            ...
            if missing_clients_to_be_removed:
        ...
        while True:
            ...
            if msg.type == "client_ready":
                if not msg.data:
                    ...
                    continue
                elif msg.data not in [__version__, -1]:
                    if msg.data[:4] == __version__[:4]:
                        logger.debug(
...
class WorkerRunner(DistributedRunner):
    ...
    def __init__(self, environment: "Environment", master_host: str, master_port: int) -> None:
        ...
        self.client_id = f"{socket.gethostname()}_{uuid4().hex}"
    ...
    def worker(self) -> NoReturn:
        ...
        while True:
            ...
            if msg.type == "ack":
                ...
                if self.worker_state not in [STATE_RUNNING, STATE_SPAWNING]:
                    self.stats.clear_all()
                    self.exceptions = {}
                    self.cpu_warning_emitted = False
                    self.worker_cpu_warning_emitted = False
